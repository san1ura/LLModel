"""
Optimized training pipeline for the transformer model
...
class OptimizedTrainer:
    """
    Optimized trainer class supporting various training approaches
    """
    ...
    def __init__(
        ...
        lr: float = 1e-4,  # Increased default learning rate
        weight_decay: float = 0.1,
        warmup_steps: int = 500,  # Reduced warmup steps for smaller datasets
        total_steps: int = 10000,  # Increased total steps to allow proper learning
        log_interval: int = 10,
        save_interval: int = 1000,  # Less frequent saving to reduce overhead
        eval_interval: int = 500,  # Less frequent evaluation to reduce overhead
        ...
        early_stopping_patience: int = 5,  # Increased patience to avoid premature stopping
    ...
    def compute_loss(self, logits, labels):
        ...
        if len(logits.shape) != len(labels.shape) + 1:
            raise ValueError(f"Logits and labels have incompatible shapes: "
        ...
        # Ensure shapes are compatible after shifting
        if shift_logits.size(1) != shift_labels.size(1):
            # Adjust the shorter tensor to match the longer one
    ...
    def train_step(self, batch):
        ...
        if self.use_amp and str(self.device) == "cuda":
            with autocast("cuda"):
                ...
                # Ensure logits and labels have compatible shapes
                if len(logits.shape) != len(labels.shape) + 1:
                    # If there's a mismatch, ensure proper reshaping
                    if len(logits.shape) == 3 and len(labels.shape) == 2:
                        ...
                        # This is for a different case, but shouldn't happen in our context
                        pass
                    else:
                        # Log the shape mismatch for debugging
                        print(f"Shape mismatch in forward pass: logits {logits.shape}, labels {labels.shape}")
                        raise ValueError(f"Shape mismatch between logits and labels")
    ...
    def save_checkpoint(self, step: int, eval_loss: float = None):
        ...
        self.logger.info(f"Checkpoint saved to: {checkpoint_path} as checkpoint_{step}.pt")
    ...
    def train(self, epochs: int = None):
        ...
        if hasattr(self.train_data, "dataset") and hasattr(
            ...
            for _ in self.train_data:
                dataset_size += 1
                if dataset_size >= 100000:  # Limit estimation to avoid long waits
                    break
        ...
        if epochs is not None:
            total_steps = max(1000, epochs * steps_per_epoch)  # Ensure at least 1000 steps
        else:
            # If no epochs specified, use the existing total_steps
            total_steps = self.total_steps
            epochs = max(1, total_steps // steps_per_epoch) if steps_per_epoch > 0 else 1
        ...
        for epoch in range(epochs if epochs is not None else 1):
            epoch_step = 0

            # Process one epoch
            while epoch_step < steps_per_epoch and completed_steps < total_steps:
                try:
                    # Get batch
                    try:
                    ...
                    # Evaluation
                    if self.eval_data and self.global_step > 0 and self.global_step % self.eval_interval == 0:
                        self.logger.info(
                    ...
                    elif self.global_step > 0 and self.global_step % self.save_interval == 0:
    ...
    def generate_sample_text(self):
        ...
        try:
            ...
            if hasattr(self, "tokenizer") and self.tokenizer is not None:
                # Use the tokenizer to encode the sample prompt
                ...
                for i, token_id in enumerate(prompt_tokens):
                    if token_id >= vocab_size:
                        self.logger.warning(f"Token ID {token_id} at position {i} exceeds model vocab size {vocab_size}")
                        # Replace with unknown token ID (usually 0 or tokenizer.unk_token_id)
                        prompt_tokens[i] = self.tokenizer.unk_token_id if hasattr(self.tokenizer, 'unk_token_id') else 0

                self.logger.info(f"Sample prompt tokens: {prompt_tokens}")
            else:
                # Use simple token IDs based on vocab size
                prompt_tokens = [1, 2, 3, 4, 5]  # Simple example tokens
                self.logger.warning("Tokenizer not available, using default tokens")
            ...
            # Validate that all input IDs are within bounds before generation
            vocab_size = self.model.config.vocab_size
            max_input_id = torch.max(input_ids).item()
            if max_input_id >= vocab_size:
                self.logger.error(f"Input token IDs contain values >= vocab_size ({vocab_size}). Max token ID: {max_input_id}")
                return
...
class PreTrainer(OptimizedTrainer):
    """
...
class SFTTrainer(OptimizedTrainer):
    """
...
class RLHFTrainer(OptimizedTrainer):
    """
...
class DPOTrainer(OptimizedTrainer):
    """
