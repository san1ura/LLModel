"""
Test for the tokenizer functionality
"""
import unittest
import tempfile
import os
from tokenizer.train_tokenizer import TokenizerTrainer, TokenizerWrapper
...
class TestTokenizerTrainer(unittest.TestCase):
    ...
    def test_train_from_texts(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            output_path = os.path.join(tmp_dir, "tokenizer.json")
            
            trainer = TokenizerTrainer(vocab_size=100, model_type="BPE")
            tokenizer = trainer.train_from_texts(sample_texts, output_path, min_frequency=1)
            
            # Check if tokenizer file was created
            self.assertTrue(os.path.exists(output_path))
            
            # Check if we can load the tokenizer
            loaded_tokenizer = TokenizerWrapper(tokenizer_path=output_path)
    ...
    def test_different_model_types(self):
        ...
        sample_texts = ["test text", "more text"]
        
        for model_type in ["BPE", "WordPiece"]:
            with tempfile.TemporaryDirectory() as tmp_dir:
                output_path = os.path.join(tmp_dir, f"tokenizer_{model_type.lower()}.json")
                
                trainer = TokenizerTrainer(vocab_size=50, model_type=model_type)
                trainer.train_from_texts(sample_texts, output_path, min_frequency=1)
...
class TestTokenizerWrapper(unittest.TestCase):
    def setUp(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            trainer = TokenizerTrainer(vocab_size=100, model_type="BPE")
            trainer.train_from_texts(sample_texts, self.tokenizer_path, min_frequency=1)
    ...
    def test_save_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            save_path = os.path.join(tmp_dir, "saved_tokenizer.json")
            ...
            # Load the tokenizer
            loaded_tokenizer = TokenizerWrapper(tokenizer_path=save_path)
...
class TestTokenizerUtilities(unittest.TestCase):
    def test_create_tokenizer_from_vocab(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            output_path = os.path.join(tmp_dir, "vocab_tokenizer.json")

            tokenizer = create_tokenizer_from_vocab(vocab, output_path)
            loaded_tokenizer = TokenizerWrapper.from_pretrained(output_path)

            self.assertEqual(loaded_tokenizer.get_vocab_size(), len(vocab))
    ...
    def test_train_default_tokenizer(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            output_path = os.path.join(tmp_dir, "default_tokenizer.json")
